...
local function worker(user_args)
    ...
    spotify_widget = wibox.widget {
        ...
        set_status = function(self, is_playing)
            if is_playing then 
                curIcon = bg1 .. play_icon .. bg2
            else
                curIcon = bg1 .. pause_icon .. bg2
            end
        ...
        set_text = function(self, artist, song)
            ...
            local str = bg1..' '..curIcon..' '.. artist_to_display .. ' - ' .. title_to_display .. ' '..bg2
            if self:get_children_by_id('titlew')[1]:get_markup() ~= str then
                self:get_children_by_id('titlew')[1]:set_markup(str)
            end
        end
    }
